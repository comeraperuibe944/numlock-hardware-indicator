#!/usr/bin/env python3
"""
Num Lock CPU Activity Indicator (Ethernet / RJ45 style)
======================================================
Visualizes CPU load on the physical Num Lock LED(s):
- Idle / Normal state: Solid ON (steady, quiet light).
- Critical load state: Rapid micro-flickers ("ruídos no brilho")
...
CONFIG_FILE = "/etc/numlock-cpu.conf"
SERVICE_NAME = "numlock-cpu.service"
SYSTEMD_DIR = "/etc/systemd/system"
SCRIPT_PATH = os.path.abspath(__file__)

DEFAULT_THRESHOLD = 60.0    # Only flicker when CPU exceeds 60% load
...
def load_config() -> dict:
    """Loads configuration from /etc/numlock-cpu.conf if present."""
    cfg = {
...
def save_config(threshold: float, sensitivity: float, pulse_ms: int, idle_state: str):
    """Saves configuration to /etc/numlock-cpu.conf."""
    content = f"""# Num Lock CPU Indicator Configuration
# Threshold percentage (0-100). Only flicker when CPU exceeds this load.
...
class NumLockMonitor:
    def __init__(
        ...
        # 25ms tick base: pulse_extra_ticks = 0 means 1 tick (25ms)
        ...
        # Persistent /proc/stat file descriptor
        self.proc_stat_fd = os.open("/proc/stat", os.O_RDONLY)
    ...
    def read_cpu_counters(self) -> Tuple[int, int]:
        """Reads cumulative busy & total CPU counters from /proc/stat."""
        try:
            os.lseek(self.proc_stat_fd, 0, os.SEEK_SET)
            data = os.read(self.proc_stat_fd, 512).decode("ascii", errors="ignore")
            first_line = data.split("\n", 1)[0]
            fields = [int(x) for x in first_line.split()[1:8]]
            user, nice, system, idle, iowait, irq, softirq = fields[:7]
    ...
    def run(self, max_duration: Optional[float] = None):
        ...
        try:
            while self.running:
                ...
                # Read CPU stats
                busy, total = self.read_cpu_counters()
                d_busy = busy - prev_busy
                d_total = total - prev_total
                prev_busy, prev_total = busy, total

                instant_load = (d_busy / d_total) if d_total > 0 else 0.0
                # Exponential moving average filter (smoothes out 5ms micro-spikes)
                smooth_load = 0.65 * smooth_load + 0.35 * instant_load

                # Determine LED state (RJ45 activity style only above critical threshold)
                if pulse_remaining > 0:
                    pulse_remaining -= 1
                    # In pulse: micro-flicker OFF
                    new_state = 0 if self.idle_on else 1
                else:
                    # ONLY flicker if load exceeds critical threshold
                    if smooth_load >= self.threshold:
                        ...
                        # Flicker probability: scales up with load above threshold
                ...
                # Adaptive sleep:
                # When load is well below threshold, sleep 80ms to conserve CPU (~0.1% overhead)
                # When approaching or above threshold, tick at 25ms (40Hz) for crisp responsive flickers
    ...
    def cleanup(self):
        ...
        if self.proc_stat_fd >= 0:
            try:
                os.close(self.proc_stat_fd)
            except OSError:
                pass
            self.proc_stat_fd = -1
...
Description=Num Lock CPU Activity Indicator (Ethernet-style LED Visualizer)
...
def show_status():
    cfg = load_config()
    print("=== Configuration (/etc/numlock-cpu.conf) ===")
    print(f"* Threshold:    {cfg['threshold']:.1f}% (only flickers above this CPU load)")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Num Lock CPU Activity Indicator (Ethernet RJ45-style flicker)"
    )
    parser.add_argument(
        "--test",
        nargs="?",
    ...
    parser.add_argument(
        "--status",
        action="store_true",
        help="Show active service, configuration, and LED status",
    )
    parser.add_argument(
        "--threshold",
        type=float,
        default=cfg["threshold"],
        help=f"CPU percentage threshold to start flickering (default: {cfg['threshold']}%%)",
    )
    parser.add_argument(
        "--set-threshold",
        type=float,
        metavar="PERCENT",
        help="Persist new threshold percentage to /etc/numlock-cpu.conf and reload service",
    )
    parser.add_argument(
        "--sensitivity",
        type=float,
    ...
    if args.test is not None:
        duration = args.test
        print(f"[*] Starting Num Lock CPU visualizer test for {duration} seconds...")
        print(f"[*] Threshold: {args.threshold:.1f}% (Below this: SOLID ON. Above: RJ45-style FLICKER)")
        print("[*] Generating a 3-second heavy CPU burst halfway through to demonstrate critical state...")
    ...
    print(f"Num Lock CPU Activity Monitor running (Threshold: {args.threshold:.1f}%). Press Ctrl+C to stop.")
    monitor.run()
